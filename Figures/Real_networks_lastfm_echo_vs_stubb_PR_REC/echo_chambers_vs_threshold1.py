...
'''echoREAL04 = np.loadtxt("Echo_chamber_ER_REF_001_fracRes=1_stubb=04.txt")
...
echoREAL[0] = np.mean(echoREAL0[:,2])
echoREAL[1] = np.mean(echoREAL01[:,2])
echoREAL[2] = np.mean(echoREAL02[:,2])
echoREAL[3] = np.mean(echoREAL03[:,2])
echoREAL[4] = 1
...
ax.errorbar(threshold, echoSBMH, yerr=np.array(errSBMH), xerr=None, c='m', fmt='o', label = r'SBM')
...
#ax.errorbar(threshold, echoSBML, yerr=np.array(errSBML), xerr=None, c='y', fmt='o', label = r'SBM, low $Q$')

ax.errorbar(threshold, echoREAL, yerr=np.array(errREAL), xerr=None, c='g', fmt='o', label = r'Last.fm')
